...
static void get_video_devices (ofGstCamData & cam_data)
{
	...
	ofLog (OF_LOG_VERBOSE, "Probing devices with udev...");
	...
	udev_list_entry_foreach(entry,list){
		...
		ofLog (OF_LOG_VERBOSE, "Found device " + vendor_id + ":" + product_id + ", getting capabilities...");

		/* vbi devices support capture capability too, but cannot be used,
		 * so detect them by device name */
		if (strstr (dev_node, "vbi"))
		{
			ofLog (OF_LOG_VERBOSE, "Skipping vbi device: %s", dev_node);
			continue;
		}
		...
		{
			ofLog (OF_LOG_VERBOSE, "Failed to open %s: %s", dev_node, strerror (errno));
			continue;
		}
		...
		if (ok < 0)
		{
			...
			if (ok < 0)
			{
				ofLog (OF_LOG_VERBOSE, "Error while probing v4l capabilities for %s: %s",
						dev_node, strerror (errno));
				close (fd);
				continue;
			}
			ofLog (OF_LOG_VERBOSE,"Detected v4l device: %s", v1cap.name);
			ofLog (OF_LOG_VERBOSE,"Device type: %d", v1cap.type);
		...
		else
		{
			...
			ofLog (OF_LOG_VERBOSE,"Detected v4l2 device: %s", v2cap.card);
			ofLog (OF_LOG_VERBOSE,"Driver: %s, version: %d", v2cap.driver, v2cap.version);
			/* g_print ("Bus info: %s\n", v2cap.bus_info); */ /* Doesn't seem anything useful */
			ofLog (OF_LOG_VERBOSE,"Capabilities: 0x%08X", v2cap.capabilities);
			if (!(cap & V4L2_CAP_VIDEO_CAPTURE))
			{
			  ofLog (OF_LOG_VERBOSE,"Device %s seems to not have the capture capability, (radio tuner?)\n"
...
//char TempDir[255] = "";
//int mypath = GetEnvironmentVariable("GST_PLUGIN_PATH", (CHAR*) &TempDir, sizeof(char)*255);
//cout << TempDir << endl;
//
//if(!SetEnvironmentVariable("GST_PLUGIN_PATH", "C:\\Program Files\\gstreamer\\lib\\gstreamer-0.102"))
//{
//// Error
//GetLastError();
//cout << "Env variable NOT set" << endl;
//}
	...
	if(!gst_inited){
		gst_init (NULL, NULL);
		gst_inited=true;
		if(bVerbose)ofLog(OF_LOG_VERBOSE,"ofGstUtils: gstreamer inited");
	}
...
bool ofGstUtils::loadMovie(string name){
	...
	if( name.find( "://" , 0) == string::npos){
		...
		bIsStream		= false;
		printf("rephrase file name as %s\n",name.c_str());
	}else{
		bIsStream		= true;
	}


	if(bVerbose)ofLog(OF_LOG_VERBOSE,"loading "+name);
	...
	if(gstPipeline == NULL)
	{ cout << "failed to make player" << endl;}
...
void ofGstUtils::setDeviceID(unsigned id){
	...
	if(camData.webcam_devices.size()>id){
		...
		ofLog(OF_LOG_VERBOSE,"ofGstUtils: selected device doesn't exist, selecting device 0");
	}
}
...
bool ofGstUtils::initGrabber(int w, int h, int framerate){
	...
	ofLog(OF_LOG_VERBOSE,"ofGstUtils: selected format: " + ofToString(format.width) + "x" + ofToString(format.height) + " " + format.mimetype + " framerate: " + ofToString(format.choosen_framerate.numerator) + "/" + ofToString(format.choosen_framerate.denominator));
	...
	ofLog(OF_LOG_VERBOSE, "gstreamer pipeline: %s", pipeline_string);
...
bool ofGstUtils::allocate(){
	...
	if(!bIsCamera){
		...
		if(!gst_element_query_duration(gstPipeline,&format,&durationNanos))
			ofLog(OF_LOG_VERBOSE,"GStreamer: cannot query time duration");
	...
	if (bIsCamera) {
		...
		if(GstPad* pad = gst_element_get_static_pad(gstSink, "sink")){
			...
			if(framerate && GST_VALUE_HOLDS_FRACTION (framerate)){
				...
				if(bVerbose)ofLog(OF_LOG_VERBOSE,"ofGstUtils: framerate: %i/%i",fps_n,fps_d);
			}else{
				ofLog(OF_LOG_VERBOSE,"Gstreamer: cannot get framerate, frame seek won't work");
			}
...
void ofGstUtils::setPosition(float pct){
	...
	if(speed>0){
		if(!gst_element_seek(GST_ELEMENT(gstPipeline),speed, 	format,
				...
				GST_SEEK_TYPE_SET,
				-1)) {
		ofLog(OF_LOG_VERBOSE,"GStreamer: unable to seek");
		}
	}else{
		if(!gst_element_seek(GST_ELEMENT(gstPipeline),speed, 	format,
				flags,
...
void ofGstUtils::setSpeed(float _speed){
	...
	if(speed>0){
		if(!gst_element_seek(GST_ELEMENT(gstPipeline),speed, 	format,
				...
				GST_SEEK_TYPE_SET,
				-1)) {
		ofLog(OF_LOG_VERBOSE,"GStreamer: unable to change speed");
		}
	}else{
		if(!gst_element_seek(GST_ELEMENT(gstPipeline),speed, 	format,
				flags,
	...
	if(bVerbose)ofLog(OF_LOG_VERBOSE,"Gstreamer: speed change to %f", speed);
...
GST_MESSAGE_TYPE_NAME(msg);
		...
		if(bVerbose)ofLog(OF_LOG_VERBOSE,"GStreamer: Got %s message", GST_MESSAGE_TYPE_NAME(msg));
		...
		switch (GST_MESSAGE_TYPE (msg)) {
			...
			case GST_MESSAGE_BUFFERING:
				...
				if(bVerbose)ofLog(OF_LOG_VERBOSE,"GStreamer: buffering %i\%", pctBuffered);
			...
			case GST_MESSAGE_DURATION:{
				...
				//if(!
					...
					//	)
					//ofLog(OF_LOG_VERBOSE,"GStreamer: cannot query duration");
			}break;
			...
			case GST_MESSAGE_STATE_CHANGED:{
				...
				if(bVerbose)ofLog(OF_LOG_VERBOSE,"GStreamer: state changed from " + getName(oldstate) + " to " + getName(newstate) + " (" + getName(pendstate) + ")");
			}break;

			case GST_MESSAGE_ASYNC_DONE:
				gstData.speed=speed;
				if(bVerbose)ofLog(OF_LOG_VERBOSE,"GStreamer: async done");
			break;
			...
			case GST_MESSAGE_EOS:
				if(bVerbose)ofLog(OF_LOG_VERBOSE,"GStreamer: end of the stream.");
				bIsMovieDone = true;
			...
			default:
				if(bVerbose)ofLog(OF_LOG_VERBOSE,"GStreamer: unhandled message");
			break;
		}
